...
from telegram import notify_new_data, notify_error
...
#                 función tryLogin para intentar el inicio de sesión
# ───────────────────────────────────────────────────────────────────
def getToken(session):
    """
    Obtiene el Token que usa el form del Login. Si se logra, llama
    a la función para intentar el inicio de sesión con las credenciales.
    """
    ...
    if resp.status_code == 200:
        ...
        notify_error("No se identifico el token en la página de login.")
...
#                   credenciales y con el Token. El éxito se logra si se
...
def tryLogin(session):
    ...
    if resp.status_code == 200 and "Estimado Docente" in resp.text:
        ...
        notify_error("Login fallido, credenciales invalidas!")
...
def downloadData(session):
    ...
    Si se logra, entonces llama a la función checkIfNewData().
    """
    # Peticion GET
    ...
    if resp.status_code == 200:
        ...
        if main_div:
            ...
            notify_error("No se encontró el div con role='main'.")
    else:
        logging.error(f"Error al realizar la solicitud: {resp.status_code}")
        notify_error("Error al realizar la solicitud de la página del curso.")
